use web3::{contract::{Contract, Options}, transports::Http, types::H160};
...
impl Validator {
    ...
    pub async fn is_valid(&self, key: String) -> bool {
        let result: bool = self.contract.query("isValid", (key.clone(),), None, Options::default(), None).await.unwrap();
    ...
    pub fn new() -> Self {
        ...
        let infura_project = std::env::var("INFURA_PROJECT").expect("Please set the INFURA_PROJECT env variable");
        let network_url = format!("https://kovan.infura.io/v3/{}", infura_project);
        let vault_address = {
            let string = std::env::var("VAULT_ADDRESS").expect("Please set VAULT_ADDRESS env variable");
